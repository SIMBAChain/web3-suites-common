    ...
    ERC721Pausable
{
    ...
    */
     ...
     */
    function mint(address to, uint256 tokenId, bytes32 contentHash, bytes32 name, bytes32[4] memory description, bytes32[4] memory image, bytes32 imageHash, bool salvage) public virtual onlyImplementation {
        ...
        _mint(to, tokenId);
        _setMetadata(tokenId, name, contentHash, description, image, imageHash, salvage);
    ...
    function _setMetadata(uint256 tokenId, bytes32 name, bytes32 contentHash, bytes32[4] memory description, bytes32[4] memory image, bytes32 imageHash, bool salvage) internal {
        require(hasRole(MINTER_ROLE, _msgSender()), "needs minter role");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        require(_exists(tokenId), "token does not exist");
        require(_metadata[tokenId].exists == true, "metadata does not exist");
        return MetadataLib.makeUri(_metadata[tokenId].name, _metadata[tokenId].contentHash, _metadata[tokenId].description, _metadata[tokenId].image, _metadata[tokenId].imageHash, _metadata[tokenId].salvage);
    }
