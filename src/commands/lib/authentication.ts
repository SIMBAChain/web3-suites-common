...
/**
 * This class handles our login for azureb2c device login
 */
 class AzureHandler {
    private config: Configstore;
    private projectConfig: Configstore;
    ...
    constructor(
        ...
        // authInfo in simba.json should be set here
        // we should not be hardcoding authURL and/or realm in simba.json anymore
    ...
    // private async getAuthInfo(): Promise<any> {
    //     let baseURL = this.projectConfig.get("baseURL") ? this.projectConfig.get("baseURL") : this.projectConfig.get("baseUrl");
    //     if (!baseURL) {
    //         SimbaConfig.log.error(`${chalk.redBright(`\nsimba: no value set for 'baseURL' in simba.json.`)}`);
    //         return;
    //     }
    //     baseURL = handleV2(baseURL);
    //     const authInfoURL = `${baseURL}/authinfo`;
    //     const res = await axios.get(authInfoURL);
    //     const resData = res.data;
    //     SimbaConfig.log.debug(`${chalk.cyanBright(`\nsimba: resData: ${JSON.stringify(resData)}`)}`);
    //     SimbaConfig.authInfo = resData;
    // }
